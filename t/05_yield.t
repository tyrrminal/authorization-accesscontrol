...
my $r = [];
ac->roles->perform('read')->on_resource('Post')->yield(sub() { 'post' })
  ->denied(sub { push($r->@*, "d") });
is($r, ['d'], 'yield without necessary attributes');

$r = [];
ac->roles->perform('read')->on_resource('Post')->with_attributes({own => true})
  ->yield(sub() { 'post' })
  ->granted(sub($entity) { push($r->@*, $entity) });
is($r, ['post'], 'yield with necessary attributes');

$r = [];
ac->roles->perform('read')->on_resource('Post')
